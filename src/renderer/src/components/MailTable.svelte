<script lang="ts">
  ...
  import type { EmailMsg, SortKey } from '../types'
  ...
  function shortFolderLabel(folderPath: string) {
...
<div
  class="grid gap-0 p-0 border-b border-surface-active bg-surface-active/30 font-mono text-[10px] text-accent-dim uppercase tracking-widest shrink-0"
  style={gridStyle}
>
  <div
    role="button"
    ...
    onclick={() => onSort('from')}
    onkeydown={(e) => {
      if (e.key === 'Enter' || e.key === ' ') onSort('from')
    }}
  >
  ...
  <div
    use:resizable={0}
    class="cursor-col-resize hover:bg-brand/20 transition-colors w-1 bg-surface-active/50"
  ></div>
  ...
  <div
    ...
    onkeydown={(e) => {
      if (e.key === 'Enter' || e.key === ' ') onSort('subject')
    }}
  >
    <div class="flex items-center">
      <span>
        SUBJECT {#if sortKey === 'subject'}<span class="text-brand">{sortDesc ? '↓' : '↑'}</span
          >{/if}
      </span>
  ...
  <div class="p-2 flex items-center justify-end gap-2">
    <button
      ...
      onclick={() => onSort('date')}
      onkeydown={(e) => {
        if (e.key === 'Enter' || e.key === ' ') onSort('date')
      }}
    >
...
<div class="flex-1 overflow-y-auto overflow-x-hidden relative">
  {#if loading && emails.length === 0}
    <div
      class="absolute inset-0 flex items-center justify-center text-accent-dim font-mono text-[10px] animate-pulse"
    >
      CONNECTING...
    </div>
  ...
  {:else}
    {#each emails as email (email.id)}
      {@const assignedFolder = assignedFolders[email.id]}
      <div
        role="button"
        tabindex="0"
        class="grid gap-0 p-0 border-b border-surface-active cursor-pointer transition-all hover:bg-surface-hover/50 text-sm group relative
               {altPressed
          ? 'hover:cursor-cell hover:ring-1 hover:ring-brand/60 hover:ring-inset'
          : ''}
        ...
        onkeydown={(e) => {
          if (e.key === 'Enter' || e.key === ' ') onRowClick(e, email)
        }}
      >
        {#if assignedFolder}
          <div class="absolute left-0 top-0 bottom-0 w-1 bg-brand z-10"></div>
        {/if}

        <div class="p-3 truncate font-medium flex items-center gap-2">
          {#if assignedFolder}
            <span
              class="text-[11px] px-1 rounded bg-surface-active font-mono text-brand"
              title={assignedFolder}
            >
              {shortFolderLabel(assignedFolder)}
            </span>
          {/if}
          <span class="truncate {selectedIds.has(email.id) ? 'text-brand' : 'text-accent/90'}"
            >{email.from}</span
          >
        </div>
        <div class="bg-transparent w-1"></div>

        <div class="p-3 truncate flex items-center gap-2">
          <span class="truncate {selectedIds.has(email.id) ? 'text-accent' : 'text-accent/80'}"
        ...
        <div
          class="p-3 text-right text-accent-dim font-mono text-[11px] whitespace-nowrap self-center"
        >
